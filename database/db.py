...
async def add_or_update_user(telegram_id: int, region_id: str, queue_id: str):
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("""
        ...
        """, (telegram_id, region_id, queue_id))
...
async def get_users_by_queue(region_id: str, queue_id: str) -> List[int]:
    async with aiosqlite.connect(DB_PATH) as db:
        async with db.execute("SELECT telegram_id FROM users WHERE region_id = ? AND queue_id = ?", (region_id, queue_id)) as cursor:
